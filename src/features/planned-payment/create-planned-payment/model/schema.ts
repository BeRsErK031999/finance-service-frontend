...
import {
  ...
  type CreatePlannedPaymentRequest,
} from '../../../../entities/planned-payment/model/types'

const amountPattern = /^\d+(?:[.,]\d{1,2})?$/
const dateInputPattern = /^\d{4}-\d{2}-\d{2}$/

export const createPlannedPaymentFormSchema = z
  .object({
    name: z.string().trim().min(1, 'Укажите название поступления'),
    amount: z
      .string()
      .trim()
      .min(1, 'Укажите сумму')
...
export type CreatePlannedPaymentFormValues = z.input<
  typeof createPlannedPaymentFormSchema
>

export const defaultCreatePlannedPaymentFormValues: CreatePlannedPaymentFormValues =
  {
    name: '',
    amount: '',
...
export function mapCreatePlannedPaymentFormValuesToRequest(
  values: CreatePlannedPaymentFormValues,
